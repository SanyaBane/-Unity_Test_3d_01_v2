...
namespace UnityEditor
{
    public abstract class BaseShaderGUI : ShaderGUI
    {
        ...
        protected class Styles
        {
            ...
            public static readonly GUIContent stencilRefSlider = new GUIContent("Stencil Ref",
                "TODO.");
            ...
            public static readonly GUIContent stencilPassText = new GUIContent("Stencil Pass",
                "TODO.");
            public static readonly GUIContent stencilFailText = new GUIContent("Stencil Fail",
                "TODO.");
            public static readonly GUIContent stencilZFailText = new GUIContent("Stencil ZFail",
                "TODO.");
        }
        ...
        protected MaterialProperty stencilFailProp { get; set; }
        protected MaterialProperty stencilZFailProp { get; set; }
        ...
        public virtual void FindProperties(MaterialProperty[] properties)
        {
            ...
            stencilRefProp = FindProperty("_StencilRef", properties, false);
            stencilCompProp = FindProperty("_StencilComp", properties, false);
            stencilPassProp = FindProperty("_StencilPass", properties, false);
            stencilFailProp = FindProperty("_StencilFail", properties, false);
            stencilZFailProp = FindProperty("_StencilZFail", properties, false);
        }
        ...
        public virtual void DrawAdvancedOptions(Material material)
        {
            ...
            if (stencilRefProp != null)
            {
                ...
                var queue = EditorGUILayout.IntSlider(Styles.stencilRefSlider, (int)stencilRefProp.floatValue, 0, 255);
                if (EditorGUI.EndChangeCheck())
                    stencilRefProp.floatValue = queue;
                EditorGUI.showMixedValue = false;
            }
            ...
            if (stencilCompProp != null)
            {
                ...
                var stencilComp = (UnityEngine.Rendering.CompareFunction)stencilCompProp.floatValue;
                stencilComp = (UnityEngine.Rendering.CompareFunction)EditorGUILayout.EnumPopup(Styles.stencilCompText, stencilComp);
                if (EditorGUI.EndChangeCheck())
                {
                    //materialEditor.RegisterPropertyChangeUndo(Styles.stencilCompText.text);
                    stencilCompProp.floatValue = (float)stencilComp;
                }
            ...
            if (stencilPassProp != null)
            {
                ...
                var tmp = (UnityEngine.Rendering.StencilOp)stencilPassProp.floatValue;
                tmp = (UnityEngine.Rendering.StencilOp)EditorGUILayout.EnumPopup(Styles.stencilPassText, tmp);
                if (EditorGUI.EndChangeCheck())
                {
                    //materialEditor.RegisterPropertyChangeUndo(Styles.stencilCompText.text);
                    stencilPassProp.floatValue = (float)tmp;
                }

                EditorGUI.showMixedValue = false;
            }

            if (stencilFailProp != null)
            {
                EditorGUI.BeginChangeCheck();
                EditorGUI.showMixedValue = stencilFailProp.hasMixedValue;
                var tmp = (UnityEngine.Rendering.StencilOp)stencilFailProp.floatValue;
                tmp = (UnityEngine.Rendering.StencilOp)EditorGUILayout.EnumPopup(Styles.stencilFailText, tmp);
                if (EditorGUI.EndChangeCheck())
                {
                    //materialEditor.RegisterPropertyChangeUndo(Styles.stencilCompText.text);
                    stencilFailProp.floatValue = (float)tmp;
                }

                EditorGUI.showMixedValue = false;
            }

            if (stencilZFailProp != null)
            {
                EditorGUI.BeginChangeCheck();
                EditorGUI.showMixedValue = stencilZFailProp.hasMixedValue;
                var tmp = (UnityEngine.Rendering.StencilOp)stencilZFailProp.floatValue;
                tmp = (UnityEngine.Rendering.StencilOp)EditorGUILayout.EnumPopup(Styles.stencilZFailText, tmp);
                if (EditorGUI.EndChangeCheck())
                {
                    //materialEditor.RegisterPropertyChangeUndo(Styles.stencilCompText.text);
                    stencilZFailProp.floatValue = (float)tmp;
                }
