		...
		protected override void MovementUpdateInternal (float deltaTime, out Vector3 nextPosition, out Quaternion nextRotation) {
			...
			bool stopped = isStopped 
			               || (reachedDestination && (whenCloseToDestination == CloseToDestinationMode.Stop 
			                                          || whenCloseToDestination == CloseToDestinationMode.InstantStop));
			...
			if (interpolator.valid && !stopped) {
				...
				} else if (reachedEndOfPath && whenCloseToDestination == CloseToDestinationMode.InstantStop) {
				...
				slowdown = 1;
				if (whenCloseToDestination == CloseToDestinationMode.InstantStop) {
					velocity2D = new Vector2(0, 0);
				} else {
					// Slow down as quickly as possible
					velocity2D -= Vector2.ClampMagnitude(velocity2D, currentAcceleration * deltaTime);
